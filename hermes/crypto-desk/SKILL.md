...
| `/crypto-desk analyze SYMBOL` | `desk --json analyze SYMBOL \| python -m crypto_desk.notifications analysis` |
...
- Accept only `BTCUSDT, ETHUSDT, BNBUSDT, SOLUSDT, SUIUSDT`. Refuse any other symbol before
  calling the CLI.
...
- For `analyze`, return the formatter output unchanged so price, analysis sections,
  risk levels, ticket status and run ID stay consistent and raw JSON is never exposed.
